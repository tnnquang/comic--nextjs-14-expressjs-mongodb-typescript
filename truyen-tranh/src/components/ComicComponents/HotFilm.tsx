...
import { formatNumber, returnDefaultImageURL_v2, toStar } from "@/common/utils";
import { BASE_URL_API, fetchOrigin, GET_HOT_COMIC } from "@/common/constant";

export default async function HotFilmComponent() {
  const res = await fetch(`${BASE_URL_API}${GET_HOT_COMIC}`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      category: ["Uncensored Leak", "Uncensored", "Amateur"],
      limit: 12,
    }),
          ...
          dataFilm.map(async (film: IFilm) => {
            ...
            return (
              <Link
                ...
                key={`b${film.slug}`}
                href={`/detail/${film.slug}`}
                prefetch={false}
              >
                <span className="relative block h-[180px] w-full max-w-[280px] overflow-hidden rounded-md border border-blue-200 2lg:h-[87px] 2lg:max-w-[75px] 2lg:rounded-md">
                  <Image
                    src={imgSrc}
                    fill
                    sizes="100vw"
                    alt={film.code as string}
                ...
                <div className="info-data">
                  <p className="title mb-0.5 md:mb-2 text-sm capitalize line-clamp-1">
                    {film.code}
                  </p>
                  <p className="year text-xs opacity-80">{film.year_release}</p>
                  <p className="rates 2lg: absolute left-1/2 top-2 flex -translate-x-1/2 items-center gap-1 rounded-bl-md rounded-br-md bg-blueSecondary bg-opacity-80 px-3 py-1.5 text-xs text-white 2lg:left-auto 2lg:top-auto 2lg:translate-x-0 2lg:bg-transparent 2lg:p-0 w-[90%] justify-center lg:justify-start">
                    {/* {toStar(film.rate)} <IoIosStar size={16} color="orange" /> */}
                    ~ {formatNumber(film.views as number)} views
                  </p>
                </div>
              </Link>
            );
...
export async function CommingSoonComponent_v2() {
  const res = await fetch(`${BASE_URL_API}${GET_HOT_COMIC}`, {
    ...
    next: { revalidate: 900, tags: ["hot-film"] },
  }).then((e) => e.json());
  const dataFilm = res.result;
