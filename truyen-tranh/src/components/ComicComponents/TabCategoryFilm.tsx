...
export default async function TabCategoryFilmComponent({
  ...
  return (
    <div className="list-film-tab-category w-full">
          ...
          <Link
            className={`inline-block border-b-2 pb-2 ${
              tabSelected.value.toLowerCase() === e.value.toLowerCase()
                ...
                : "border-b-[#5142fc7f] text-white font-semibold"
            }`}
            key={e.value + "kk"}
            role="button"
            tabIndex={1}
      ...
      <div className="list-film-container mb-10 mt-3 lg:mt-6 flex flex-wrap items-start gap-2">
        <ListComicItemComponent listFilm={dataFilm} />
      </div>
    </div>
  );
}
