...
export function HeaderAdsComponent({
  ...
  return dataAds && dataAds.length > 0 ? (
    <section className="mx-auto w-full max-w-[1800px] 2lg:px-4 relative z-10">
...
export function Row1AdsComponent({
  ...
  return dataAds && dataAds.length > 0 ? (
    <section className="row1-bn any-data-wrapper mx-auto my-3 w-full relative z-[11]">
      {dataAds.map((item: any, index) =>
        item.mode === "dual" ? (
          <div
            className="relative flex w-full flex-col items-center md:flex-row"
  ...
  return dataAds && dataAds?.length > 0 ? (
    show ? (
      <div
        ...
        className="bg-slate-300 backdrop-blur-[2px] fixed left-0 top-0 z-[1000] flex h-full w-full items-center justify-center bg-black bg-opacity-25"
      >
